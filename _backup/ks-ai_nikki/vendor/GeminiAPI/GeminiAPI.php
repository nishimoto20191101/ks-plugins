...
use GeminiAPI\Client as KS_GeminiAPI_Client;
use GeminiAPI\Resources\Parts\TextPart  as KS_GeminiAPI_TextPart;
//use GeminiAPI\Resources\Parts\ImagePart as KS_GeminiAPI_ImagePart; //画像入力
...
class Ks_GeminiAPI {
  ...
  function __construct($key){
    ...
    return $this->AiClient = new KS_GeminiAPI_Client($this->GeminiApiKey);
  }
  //#### 生成AIサービスからの返答 \vendor\GeminiAPI\src\Resources\Content.php ####//
  public function get_anser($prompt){
    $result = $this->AiClient->geminiPro()->generateContent( new KS_GeminiAPI_TextPart($prompt) )->text();
    if( empty($result) && $this->anser_err < 5 ){
      $this->anser_err++;
      $result = $this->get_anser($prompt);
    }
    $this->anser_cnt = 0;
    return $result;
  }
  //#### 生成AIサービスからの返答（画像入力） \vendor\GeminiAPI\src\Resources\Content.php ####//
  /*public function get_anser_image($image_url, $mimeType='image/jpeg'){
    $result = $this->AiClient->geminiPro()->generateContent( new KS_GeminiAPI_ImagePart($mimeType, base64_encode(file_get_contents($image_url))) )->text();
    if( empty($result) && $this->anser_err < 5 ){
      $this->anser_err++;
...
TEXT;
      ...
      case 'title':
        if(in_array($data["text_form"], ["ニュース"])){
          ...
          $constraints = "6.{$date}以降の最も大事なニュースに絞っていること\n";
        }else{
          $constraints = "";
        }
        $result =<<<TEXT
...
TEXT;
        //$result = "{$data["editor_prompt"]}\n\n".wp_date("Y-m-d H:i:s")."に作成する[{$data["text_form"]}タイトル]を記事を役割・目標、視点・対象、制約条件、処理手順(CoT)、入力文、出力文を考慮して１つ作成して";
        break;
      ################################################################################################
      case 'post':
...
TEXT;
        ...
        if( is_array($data["reference"]) ){
          ...
          foreach( $data["reference"] as $key => $val){
            ...
            $print_reference .=<<<HTML
            <li><a href="{$val["url"]}" target="_blank" rel="noopener">{$val["title"]}</a></li>
HTML;
          }
          $reference .= "\n";
